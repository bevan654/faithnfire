...
module.exports = async function handler(req, res) {
  ...
  const { id, first_name, last_name, email, parish, state, team_name, captain, players } = req.body;
  ...
  // Compute changes
  const changes = [];
  const fields = { first_name, last_name, email, parish, state, team_name, captain };
  ...
  const { data, error } = await sb
    .from('girls_registrations')
    .update({ first_name, last_name, email, parish, state, team_name, captain, players })
    .eq('id', oldData.id)
    .select();
  ...
  if (changes.length > 0) {
    const { error: logErr } = await sb.from('audit_log').insert({
      ...
      team_name: '[Girls] ' + (team_name || oldData.team_name),
      action: 'edit',
      changes
    });
