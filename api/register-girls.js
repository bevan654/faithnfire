...
module.exports = async function handler(req, res) {
  ...
  const { first_name, last_name, email, parish, state, team_name, captain, players } = req.body;
  ...
  const { data, error } = await sb
    .from('girls_registrations')
    .insert({ first_name, last_name, email, parish, state, team_name, captain, players })
    .select();
